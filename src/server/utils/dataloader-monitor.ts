...
export class DataLoaderMonitor {
   ...
   * 记录 DataLoader 请求统计
   */
  static recordRequest(loaderName: string, isBatch: boolean, batchSize?: number) {
   ...
   * 记录缓存命中/未命中
   */
  static recordCacheResult(loaderName: string, isHit: boolean) {
    const stats = this.requestStats.get(loaderName);
    if (stats) {
      if (isHit) {
        stats.cacheHits++;
...
export function getDataLoaderHealth(context?: DataLoaderContext) {
  ...
  return {
    ...
    contextStats: context ? getDataLoaderStats(context) : null,
  };
}
