 ...
 * 生成控制器对应的多语言文件
 */
export class I18nGenerator extends BaseGenerator {
  /**
   * 生成中文翻译文件
   */
   ...
   * 生成占位符翻译
   */
  private generatePlaceholderTranslations(formFields: FieldDefinition[], locale: string): Record<string, string> {
    const translations: Record<string, string> = {}
   ...
   * 获取字段翻译
   */
  private getFieldTranslation(field: FieldDefinition, locale: string): string {
   ...
   * 获取操作列翻译
   */
  private getActionsTranslation(locale: string): string {
    switch (locale) {
      case 'zh-CN':
        return '操作'
      case 'en-US':
   ...
   * 简单的中文到英文翻译映射
   */
  private translateChineseToEnglish(chinese: string): string | null {
    const translations: Record<string, string> = {
      用户名: 'Username',
      用户: 'User',
      邮箱: 'Email',
   ...
   * 简单的中文到日文翻译映射
   */
  private translateChineseToJapanese(chinese: string): string | null {
    const translations: Record<string, string> = {
      用户名: 'ユーザー名',
      用户: 'ユーザー',
      邮箱: 'メール',
