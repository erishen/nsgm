...
export class ArgumentParser {
   ...
   * 解析命令行参数
   */
  static parse(args: string[] = process.argv.slice(2)): ParsedArgs {
    if (args.length === 0) {
   ...
   * 解析位置参数（保持向后兼容）
   */
  private static parsePositionalArgs(command: string, args: string[], options: CommandOptions): void {
    ...
    args.forEach((arg, index) => {
      ...
      if (createCommands.includes(command)) {
        switch (index) {
          case 0:
            if (!options.controller) options.controller = arg
            break
          case 1:
            if (!options.action) options.action = arg
            break
          case 2:
            if (!options.dictionary) options.dictionary = arg
            break
        }
